...
# alias factorize="$ZDOTDIR/node_engine $PATH/factorize"
...
add-zsh-hook precmd set_starship_prompt

# 3. 빌트인 명령어 제한
disable pwd pushd popd dirs
disable set which whence where type
disable exec eval builtin command zmodload
disable unsetopt setopt
disable zcompile vared
...
# 4. 배너 실행
if [ -f "$HOME/.local/bin/welcome" ]; then

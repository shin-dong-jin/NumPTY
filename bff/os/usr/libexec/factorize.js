...
if (!number || !/^\d+$/.test(number) || number === "0") {
  console.error("\x1b[1;31mUsage: factorize <positive integer>\x1b[0m");
  process.exit(1);
}

const formattedNumber = BigInt(number).toLocaleString();
  ...
  spinnerInterval = setInterval(() => {
    ...
    if (currentState === "PENDING") {
      ...
      process.stdout.write(
        `\r\x1b[1;36m[ ${char} ] CALCULATING factors of ${formattedNumber}${dots} \x1b[0m`,
      );
    }
    idx++;
  }, 100);
...
const finalize = (taskData) => {
  ...
  if (taskData.status === "COMPLETED") {
    const formattedResult =
      ...
      " = " +
      taskData.result
        .replace(/\d+/g, (n) => BigInt(n).toLocaleString())
        .replace(/\*/g, " * ");
...
const execute = async () => {
  ...
  try {
    ...
    const res = await fetch(`${API_BASE}/api/tasks/factorize`, {
      method: method,

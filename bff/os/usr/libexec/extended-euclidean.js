...
if (
  ...
  console.error(
    "\x1b[1;31mUsage: extended-euclidean <integer> <integer>\x1b[0m",
  );
  process.exit(1);
}
  ...
  spinnerInterval = setInterval(() => {
    ...
    if (currentState === "PENDING") {
      ...
      process.stdout.write(
        `\r\x1b[1;36m[ ${char} ] CALCULATING extended gcd of ${formattedTargetA} and ${formattedTargetB}${dots} \x1b[0m`,
      );
    }
    idx++;
  }, 100);
...
const formatResult = (coeff, multiplier, isFirst) => {
  const m = BigInt(multiplier);
  const isNeg = m < 0n;
  const absM = isNeg ? -m : m;
  const coeffStr = BigInt(coeff).toLocaleString();
  const mStr = absM.toLocaleString();

  if (isFirst) {
    return `${isNeg ? "-" : ""}${coeffStr} * ${mStr}`;
  }
  return `${isNeg ? "-" : "+"} ${coeffStr} * ${mStr}`;
};
...
const finalize = (taskData) => {
  ...
  if (taskData.status === "COMPLETED") {
    const formattedResult =
      ...
      " = " +
      formatResult(targetA, taskData.resultX, true) +
      " " +
      formatResult(targetB, taskData.resultY, false);
...
const execute = async () => {
  ...
  try {
    ...
    const res = await fetch(`${API_BASE}/api/tasks/extended-euclidean`, {
      method: method,

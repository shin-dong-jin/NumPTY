...
if (!target || !/^\d+$/.test(target)) {
  console.error("\x1b[1;31mUsage: primality-test <integer>\x1b[0m");
  process.exit(1);
}

const formattedTarget = BigInt(target).toLocaleString();
  ...
  spinnerInterval = setInterval(() => {
    ...
    if (currentState === "PENDING") {
      ...
      process.stdout.write(
        `\r\x1b[1;36m[ ${char} ] CALCULATING primality of ${formattedTarget}${dots} \x1b[0m`,
      );
    }
    idx++;
  }, 100);
...
const execute = async () => {
  ...
  try {
    ...
    const res = await fetch(`${API_BASE}/api/tasks/primality-test`, {
      method: method,
